...
<div class="-mx-4 sm:-mx-6 lg:-mx-8">
    <div class="mb-8 px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-600 mt-2">Update purchase order <?php echo e($purchaseOrder->po_number); ?></p>
                
                <!-- Date Summary -->
                <?php if($purchaseOrder->po_date || $purchaseOrder->delivery_date_requested): ?>
                    <div class="mt-3 flex flex-wrap gap-4 text-sm">
                        <?php if($purchaseOrder->po_date): ?>
                            <div class="flex items-center text-gray-600">
                                ...
                                <span>Created: <?php echo e(\Carbon\Carbon::parse($purchaseOrder->po_date)->format('M d, Y')); ?></span>
                            </div>
                        <?php endif; ?>
                        
                        <?php if($purchaseOrder->delivery_date_requested): ?>
                            <?php
                            ...
                            ?>
                            <div class="flex items-center <?php echo e($isOverdue ? 'text-red-600' : 'text-gray-600'); ?>">
                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path>
                                </svg>
                                <span>
                                    Delivery: <?php echo e($deliveryDate->format('M d, Y')); ?>

                                    <?php if($isOverdue): ?>
                                        (<?php echo e(abs($daysDiff)); ?> days overdue)
                                    <?php elseif($daysDiff == 0): ?>
                                        (Today)
                                    <?php elseif($daysDiff == 1): ?>
                                        (Tomorrow)
                                    <?php else: ?>
                                        (<?php echo e($daysDiff); ?> days remaining)
                                    <?php endif; ?>
                                </span>
                            </div>
                        <?php endif; ?>
            ...
            <div class="flex items-center space-x-3">
                <a href="<?php echo e(route('admin.purchase-orders.show', $purchaseOrder)); ?>" class="inline-flex items-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        ...
                        <div class="mt-2 text-sm text-amber-700">
                            <p>⚠️ <strong>Warning:</strong> This purchase order has already been sent to the supplier (Status: <?php echo e(\App\Models\PurchaseOrder::$statuses[$purchaseOrder->status]); ?>). Any changes you make will be logged for audit purposes and you must provide a reason for the edit.</p>
                            <?php if($purchaseOrder->sent_to_supplier_at): ?>
                                <p class="mt-1">📧 Originally sent on: <?php echo e($purchaseOrder->sent_to_supplier_at->format('M d, Y \a\t H:i')); ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <!-- Form -->
    <form action="<?php echo e(route('admin.purchase-orders.update', $purchaseOrder)); ?>" method="POST" enctype="multipart/form-data" onsubmit="return validateAndPrepareForm()">
        <?php echo csrf_field(); ?>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select id="currency" name="currency" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    <option value="AED" <?php echo e(old('currency', $purchaseOrder->currency) == 'AED' ? 'selected' : ''); ?>>AED (UAE Dirham)</option>
                                    <option value="USD" <?php echo e(old('currency', $purchaseOrder->currency) == 'USD' ? 'selected' : ''); ?>>USD (US Dollar)</option>
                                    <option value="CNY" <?php echo e(old('currency', $purchaseOrder->currency) == 'CNY' ? 'selected' : ''); ?>>CNY (Chinese Yuan)</option>
                                    <option value="HKD" <?php echo e(old('currency', $purchaseOrder->currency) == 'HKD' ? 'selected' : ''); ?>>HKD (Hong Kong Dollar)</option>
                                </select>
                            </div>
                            <div>
                                <label for="po_number" class="block text-sm font-medium text-gray-700 mb-2">PO Number</label>
                                <input type="text" id="po_number" value="<?php echo e($purchaseOrder->po_number); ?>" readonly
                                       class="block w-full rounded-md border-gray-300 bg-gray-50 shadow-sm sm:text-sm">
                            </div>
                            <div>
                                <label for="status" class="block text-sm font-medium text-gray-700 mb-2">Status</label>
                                <input type="text" id="status" value="<?php echo e(\App\Models\PurchaseOrder::$statuses[$purchaseOrder->status] ?? ucfirst($purchaseOrder->status)); ?>" readonly
                                       class="block w-full rounded-md border-gray-300 bg-gray-50 shadow-sm sm:text-sm">
                            </div>
                            <div>
                                <label for="po_date" class="block text-sm font-medium text-gray-700 mb-2">PO Date</label>
                                <input type="date" id="po_date" value="<?php echo e($purchaseOrder->po_date ? \Carbon\Carbon::parse($purchaseOrder->po_date)->format('Y-m-d') : ''); ?>" readonly
                                       class="block w-full rounded-md border-gray-300 bg-gray-50 shadow-sm sm:text-sm">
                                <?php if($purchaseOrder->po_date): ?>
                                    <p class="mt-1 text-sm text-gray-500">Created on <?php echo e(\Carbon\Carbon::parse($purchaseOrder->po_date)->format('M d, Y')); ?></p>
                                <?php endif; ?>
                            </div>
                            <div>
                                ...
                                <input type="date" id="delivery_date_requested" name="delivery_date_requested" 
                                       value="<?php echo e(old('delivery_date_requested', $purchaseOrder->delivery_date_requested ? \Carbon\Carbon::parse($purchaseOrder->delivery_date_requested)->format('Y-m-d') : '')); ?>" required
                                ...
                                <?php if($purchaseOrder->delivery_date_requested): ?>
                                    ...
                                    ?>
                                    <p class="mt-1 text-sm <?php echo e($isOverdue ? 'text-red-600' : 'text-gray-500'); ?>">
                                        Currently requested for <?php echo e($deliveryDate->format('M d, Y')); ?>
                                        ...
                                        <?php elseif($daysDiff == 1): ?>
                                            (Tomorrow)
                                        <?php else: ?>
                                            (<?php echo e($daysDiff); ?> days from now)
                                        <?php endif; ?>
                                    </p>
                                <?php endif; ?>
                            </div>
                    ...
                    <div class="p-6">
                        <div>
                            ...
                            <textarea id="edit_reason" name="edit_reason" rows="3" required
                                      ...
                                      placeholder="Please explain why you need to edit this sent purchase order..."><?php echo e(old('edit_reason')); ?></textarea>
                            <?php $__errorArgs = ['edit_reason'];
...
unset($__errorArgs, $__bag); ?>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" id="supplier_name" name="supplier_name" 
                                       value="<?php echo e(old('supplier_name', $purchaseOrder->supplier_name)); ?>" required
                                       class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                            <div>
                                ...
                                <input type="email" id="supplier_email" name="supplier_email" 
                                       value="<?php echo e(old('supplier_email', $purchaseOrder->supplier_email)); ?>"
                                       class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                            <div>
                                ...
                                <input type="text" id="supplier_phone" name="supplier_phone" 
                                       value="<?php echo e(old('supplier_phone', $purchaseOrder->supplier_phone)); ?>"
                                       class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                            <div>
                                ...
                                <textarea id="supplier_address" name="supplier_address" rows="3"
                                          class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"><?php echo e(old('supplier_address', $purchaseOrder->supplier_address)); ?></textarea>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="w-80">
                                <div class="bg-gray-50 p-4 rounded-lg">
                                    <div class="flex justify-between py-2">
                                        ...
                                        <span class="text-sm font-bold text-gray-900"><span id="currencyDisplaySub"><?php echo e($purchaseOrder->currency); ?></span> <span id="subTotal">0.00</span></span>
                                    </div>
                                    <div class="border-t border-gray-200 pt-2 mt-2">
                                        <div class="flex justify-between">
                                            <span class="text-base font-bold text-gray-900">Total:</span>
                                            <span class="text-base font-bold text-gray-900"><span id="currencyDisplay"><?php echo e($purchaseOrder->currency); ?></span> <span id="totalAmount">0.00</span></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <textarea id="description" name="description" rows="3"
                                          ...
                                          class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"><?php echo e(old('description', $purchaseOrder->description)); ?></textarea>
                            </div>
                            <div>
                                ...
                                <textarea id="terms_conditions" name="terms_conditions" rows="4"
                                          ...
                                          class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"><?php echo e(old('terms_conditions', $purchaseOrder->terms_conditions)); ?></textarea>
                            </div>
                            <div>
                                <label for="notes" class="block text-sm font-medium text-gray-700 mb-2">Notes</label>
                                <textarea id="notes" name="notes" rows="3"
                                          placeholder="Enter any additional notes"
                                          class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"><?php echo e(old('notes', $purchaseOrder->notes)); ?></textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                    ...
                    <div class="p-6">
                        ...
                        ?>
                        <?php if(count($existingAttachments) > 0): ?>
                            <div class="mt-6">
                                <h4 class="text-sm font-medium text-gray-900 mb-3">Current Attachments:</h4>
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                    <?php $__currentLoopData = $existingAttachments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $attachment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <div class="border border-gray-200 rounded-lg p-3 bg-gray-50">
                                            <div class="flex items-start space-x-3">
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <h5 class="text-sm font-medium text-gray-900 truncate" title="<?php echo e($attachment['filename'] ?? 'Attachment'); ?>">
                ...
                <div class="flex justify-end space-x-4">
                    <a href="<?php echo e(route('admin.purchase-orders.show', $purchaseOrder)); ?>" 
...
<?php echo json_encode($itemsData, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>
            ...
            <div class="relative product-dropdown-container">
                ...
                <div class="product-dropdown-list hidden">
                    ...
                    <div class="dropdown-items">
                        <?php $__currentLoopData = ($products ?? \Illuminate\Support\Collection::make([])); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="dropdown-item cursor-pointer p-3 hover:bg-gray-50 border-b border-gray-100 last:border-b-0" 
                                 ...
                                 data-price-usd="<?php echo e($product->price ?? 0); ?>"
                                ...
                                <?php if($product->price_aed ?? $product->price): ?>
                                    <div class="text-indigo-600 text-sm font-medium mt-1">AED <?php echo e(number_format($product->price_aed ?? $product->price, 2)); ?></div>
                                <?php endif; ?>
                            </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
...
function updatePriceTypeControlsByCurrency() {
    const currency = document.getElementById('currency') ? document.getElementById('currency').value : '<?php echo e($purchaseOrder->currency); ?>';
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Walid\OneDrive\Desktop\MaxMed\resources\views/admin/purchase-orders/edit.blade.php ENDPATH**/ ?>