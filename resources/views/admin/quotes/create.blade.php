                            ...
                            <div class="w-full max-w-sm">
                                <div class="rounded-lg bg-gray-50 p-4">
                                    ...
                                    <div class="flex justify-between py-2" id="installationRow" style="display: none;">
                                        ...
                                        <span id="installationAmount" class="text-sm font-semibold text-gray-900">0.00</span>
                                    </div>
    ...
    function selectProduct(item) {
        ...
        if (specifications && specifications !== '[]' && specifications !== 'W10=') { // W10= is base64 for []
            try {
                ...
                if (allSpecs.length > 0) {
                    ...
                    allSpecs.forEach((spec, index) => {
                        ...
                        const inputId = `spec_${itemCounter}_${index}`;
                        ...
                        if (spec && typeof spec === 'object' && spec.type === 'image') {
                            ...
                            // spec is a plain string; avoid injecting into HTML attributes directly
            ...
            if (specificationImageUrl && specificationImageUrl.trim() !== '') {
                ...
                const inputId = `spec_${itemCounter}_img`;
                const inputEl = document.createElement('input');
                inputEl.type = 'checkbox';
                inputEl.id = inputId;
                inputEl.className = 'mr-2 h-3 w-3 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded spec-checkbox';
                inputEl.dataset.spec = JSON.stringify(allSpecs[0]);

                const labelEl = document.createElement('label');
                labelEl.htmlFor = inputId;
                labelEl.className = 'flex-1 cursor-pointer';
                labelEl.textContent = `📷 ${allSpecs[0].value}`;
