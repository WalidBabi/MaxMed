    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 mb-6">
        ...
        <div class="group relative overflow-hidden rounded-md bg-gradient-to-br from-blue-50 to-indigo-100 p-1.5 shadow-sm ring-1 ring-blue-200/50 transition-all duration-300 hover:shadow-md hover:ring-blue-300/50">
            ...
            <div class="relative">
                ...
                <div class="mt-1">
                    <div class="space-y-0">
                        <p class="text-sm font-bold text-gray-900">{{ $totalQuotesCount ?? $quotes->total() }} Total</p>
                        <p class="text-xs font-semibold text-gray-700">{{ $pendingCountAll ?? ($quotes->where('status', 'draft')->count() + $quotes->where('status', 'sent')->count()) }} Pending</p>
                    </div>
                    <p class="text-xs text-blue-600 mt-0.5">All pages</p>
                </div>
            </div>
        </div>
            ...
            <div class="relative">
                <div class="flex items-center justify-between">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-xs text-yellow-500">Awaiting response</p>
                    </div>
                </div>
                <div class="mt-1">
                    <div class="space-y-0">
                        <p class="text-sm font-bold text-gray-900">{{ $pendingCountAll ?? ($quotes->where('status', 'draft')->count() + $quotes->where('status', 'sent')->count()) }} Draft</p>
                        <p class="text-xs font-semibold text-gray-700">{{ $quotes->where('status', 'sent')->count() }} Sent</p>
                    </div>
                    <p class="text-xs text-yellow-600 mt-0.5">Needs attention</p>
                </div>
            </div>
        </div>
        ...
        <div class="group relative overflow-hidden rounded-md bg-gradient-to-br from-green-50 to-emerald-100 p-1.5 shadow-sm ring-1 ring-green-200/50 transition-all duration-300 hover:shadow-md hover:ring-green-300/50">
            ...
            <div class="relative">
                <div class="flex items-center justify-between">
                    ...
                    <div class="text-right">
                        <p class="text-xs font-medium text-green-600 uppercase tracking-wide">Converted</p>
                        <p class="text-xs text-green-500">Success rate</p>
                    </div>
                </div>
                <div class="mt-1">
                    <div class="space-y-0">
                        <p class="text-sm font-bold text-gray-900">{{ $convertedCountAll ?? $quotes->where('status', 'invoiced')->count() }} Invoiced</p>
                        <p class="text-xs font-semibold text-gray-700">{{ $quotes->where('status', 'approved')->count() }} Approved</p>
                    </div>
                    <p class="text-xs text-green-600 mt-0.5">Revenue generated</p>
                </div>
            </div>
        </div>
        ...
        <div class="group relative overflow-hidden rounded-md bg-gradient-to-br from-purple-50 to-violet-100 p-1.5 shadow-sm ring-1 ring-purple-200/50 transition-all duration-300 hover:shadow-md hover:ring-purple-300/50">
            ...
            <div class="relative">
                <div class="flex items-center justify-between">
                    ...
                    <div class="text-right">
                        <p class="text-xs font-medium text-purple-600 uppercase tracking-wide">Value</p>
                        <p class="text-xs text-purple-500">Total amount</p>
                    </div>
                </div>
                <div class="mt-1">
                    <div class="space-y-0">
                        ...
                        @if($totalByAed > 0 && $totalByUsd > 0)
                            <p class="text-sm font-bold text-gray-900">{{ number_format($totalByAed, 0) }} AED</p>
                            <p class="text-xs font-semibold text-gray-700">{{ number_format($totalByUsd, 0) }} USD</p>
                        @elseif($totalByUsd > 0)
                            <p class="text-sm font-bold text-gray-900">{{ number_format($totalByUsd, 0) }} USD</p>
                            <p class="text-xs font-semibold text-gray-700">0 AED</p>
                        @elseif($totalByAed > 0)
                            <p class="text-sm font-bold text-gray-900">{{ number_format($totalByAed, 0) }} AED</p>
                            <p class="text-xs font-semibold text-gray-700">0 USD</p>
                        @else
                            <p class="text-sm font-bold text-gray-900">0 AED</p>
                            <p class="text-xs font-semibold text-gray-700">0 USD</p>
                        @endif
                    </div>
                    <p class="text-xs text-purple-600 mt-0.5">All currencies</p>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full divide-y divide-gray-200" style="min-width: 1000px;">
                    <thead class="bg-gray-50">
                        <tr>
                            <th scope="col" class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-20">Date</th>
                            <th scope="col" class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-32">Quote #</th>
                            <th scope="col" class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-40">Customer</th>
                        ...
                        @foreach($quotes as $quote)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <div class="text-xs font-medium text-gray-900 truncate">{{ $quote->quote_number }}</div>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <div class="text-xs font-medium text-gray-900 truncate" title="{{ $quote->customer_name }}">{{ Str::limit($quote->customer_name, 20) }}</div>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                ...
                                <td class="px-3 py-3 whitespace-nowrap text-xs font-medium text-gray-900">
                                    {{ number_format($quote->total_amount, 0) }} {{ $quote->currency ?? 'AED' }}
                                </td>
                                <td class="px-3 py-3 text-xs text-gray-900 products-column">
                                    @if(is_array($quote->items) ? count($quote->items) > 0 : $quote->items->count() > 0)
                                        <div class="space-y-1">
                                            @foreach((is_array($quote->items) ? collect($quote->items)->take(2) : $quote->items->take(2)) as $item)
                                                <div class="flex items-center space-x-1">
                                                    <div class="flex-1 min-w-0">
                                                        <div class="text-xs font-medium text-gray-900 truncate">
                                                            {{ Str::limit($item->product ? $item->product->name : $item->item_details, 25) }}
                                                        </div>
                                                        <div class="text-xs text-gray-500">
                                                            {{ number_format($item->quantity, 0) }} × {{ number_format($item->rate, 0) }}
                                                        </div>
                                                    </div>
                                                </div>
                                            @endforeach
                                            @if((is_array($quote->items) ? count($quote->items) : $quote->items->count()) > 2)
                                                <div class="text-xs text-gray-500 font-medium">
                                                    +{{ (is_array($quote->items) ? count($quote->items) : $quote->items->count()) - 2 }} more
                                                </div>
                                            @endif
                                        </div>
                                    @else
                                ...
                                <td class="px-3 py-3 text-xs text-gray-900">
                                    @if($quote->attachments && count($quote->attachments) > 0)
                                        <div class="space-y-1">
                                            @foreach((is_array($quote->attachments) ? collect($quote->attachments)->take(1) : $quote->attachments->take(1)) as $attachment)
                                                <div class="flex items-center space-x-1">
                                                    <div class="flex-shrink-0">
                                                        ...
                                                        @if($isPdf)
                                                            <div class="w-4 h-4 bg-red-100 rounded flex items-center justify-center">
                                            ...
                                            @endforeach
                                            @if(count($quote->attachments) > 1)
                                                <div class="text-xs text-gray-500 font-medium">
                                                    +{{ count($quote->attachments) - 1 }} more
                                                </div>
                                            @endif
                                        </div>
                                    @else
                                ...
                                <td class="px-3 py-3 whitespace-nowrap text-right text-xs font-medium">
                                    <div class="flex items-center justify-end space-x-1">
                                        <a href="{{ route('admin.quotes.show', $quote) }}" class="text-indigo-600 hover:text-indigo-900" title="View">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                            </svg>
                                        </a>
                                        <a href="{{ route('admin.quotes.edit', $quote) }}" class="text-green-600 hover:text-green-900" title="Edit">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                            </svg>
                                        </a>
                                        <a href="{{ route('admin.quotes.pdf', $quote) }}" class="text-red-600 hover:text-red-900" title="PDF">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Fetch possible recipient names for given email
