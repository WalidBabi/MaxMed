    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-3">
                @if(!$order->hasCashReceipts())
                    <form action="{{ route('admin.orders.quick-cash-receipt', $order) }}" method="POST" class="inline">
                        @csrf
                ...
                @else
                    <a href="{{ route('admin.cash-receipts.index', ['search' => $order->order_number]) }}" class="inline-flex items-center rounded-md bg-gray-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-gray-500">
                        <svg class="-ml-0.5 mr-1.5 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
