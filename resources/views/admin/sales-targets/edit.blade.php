...
<div class="max-w-7xl mx-auto">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Edit Sales Target</h1>
                <p class="text-gray-600 mt-2">Update sales target details</p>
            </div>
    ...
    <div class="card-hover rounded-xl bg-white shadow-sm ring-1 ring-gray-900/5">
        <div class="px-6 py-4 border-b border-gray-200">
            ...
            <p class="text-sm text-gray-600 mt-1">Update the details for this sales target</p>
        </div>
        <form method="POST" action="{{ route('admin.sales-targets.update', $salesTarget) }}" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $salesTarget->name) }}" required 
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3" 
                              ...
                              placeholder="Optional description of the target">{{ old('description', $salesTarget->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="target_type" id="target_type" required 
                        ...
                        @foreach(\App\Models\SalesTarget::TARGET_TYPES as $value => $label)
                            <option value="{{ $value }}" {{ old('target_type', $salesTarget->target_type) == $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="period_type" id="period_type" required 
                        ...
                        @foreach(\App\Models\SalesTarget::PERIOD_TYPES as $value => $label)
                            <option value="{{ $value }}" {{ old('period_type', $salesTarget->period_type) == $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $salesTarget->start_date->format('Y-m-d')) }}" required 
                ...
                <div>
                    ...
                    <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $salesTarget->end_date->format('Y-m-d')) }}" required 
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="target_amount" id="target_amount" value="{{ old('target_amount', $salesTarget->target_amount) }}" 
                ...
                <div>
                    ...
                    <select name="status" id="status" required 
                            class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('status') border-red-500 @enderror">
                        @foreach(\App\Models\SalesTarget::STATUS_OPTIONS as $value => $label)
                            <option value="{{ $value }}" {{ old('status', $salesTarget->status) == $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="assigned_to" id="assigned_to" 
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('assigned_to', $salesTarget->assigned_to) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
