                        ...
                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            <div class="flex-1">
                                ...
                                @if($item->product && $item->product->brand)
                                    <p class="text-xs text-gray-500">Brand: {{ $item->product->brand->name }}</p>
                                @endif
                                @if($item->size)
                            ...
                            <div class="text-right">
                                ...
                                <p class="text-xs text-gray-500">{{ $order->currency }} {{ number_format($item->quantity * $item->unit_price, 2) }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
                                ...
                                <div class="relative pb-8">
                                    @if(!$loop->last)
                                        <span class="absolute top-4 left-4 -ml-px h-full w-0.5 
                                            {{ $step['is_completed'] ? 'bg-indigo-600' : 'bg-gray-200' }}" 
                                    ...
                                    <div class="relative flex space-x-3">
                                        <div>
                                            <span class="h-8 w-8 rounded-full flex items-center justify-center ring-8 ring-white
                                                {{ $step['is_completed'] ? 'bg-indigo-600' : ($step['is_active'] ? 'bg-yellow-500' : 'bg-gray-200') }}">
                                                <svg class="w-4 h-4 {{ $step['is_completed'] || $step['is_active'] ? 'text-white' : 'text-gray-400' }}" 
                ...
                <div class="px-6 py-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-sm font-medium text-gray-900">{{ ucfirst(str_replace('_', ' ', $order->delivery->status)) }}</p>
                        </div>
                        ...
                        @if($order->delivery->tracking_number)
                            <div>
                                ...
                                <p class="text-sm font-medium text-gray-900">{{ $order->delivery->tracking_number }}</p>
                            </div>
                        @endif
