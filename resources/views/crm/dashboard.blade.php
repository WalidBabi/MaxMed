    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 mb-8">
        ...
        <div class="group relative overflow-hidden rounded-md bg-gradient-to-br from-blue-50 to-indigo-100 p-1.5 shadow-sm ring-1 ring-blue-200/50 transition-all duration-300 hover:shadow-md hover:ring-blue-300/50">
            ...
            <div class="relative">
                ...
                <div class="mt-1">
                    <div class="space-y-0">
                        <p class="text-sm font-bold text-gray-900">{{ number_format($totalLeads ?? 0) }} Total</p>
                        @if(($leadGrowthPercentage ?? 0) >= 0)
                            <p class="text-xs font-semibold text-green-600">+{{ $leadGrowthPercentage ?? 0 }}% growth</p>
                        @else
                            <p class="text-xs font-semibold text-red-600">{{ $leadGrowthPercentage ?? 0 }}% decline</p>
                        @endif
                    </div>
        ...
        <div class="group relative overflow-hidden rounded-md bg-gradient-to-br from-green-50 to-emerald-100 p-1.5 shadow-sm ring-1 ring-green-200/50 transition-all duration-300 hover:shadow-md hover:ring-green-300/50">
            ...
            <div class="relative">
                ...
                <div class="mt-1">
                    <div class="space-y-0">
                        <p class="text-sm font-bold text-gray-900">{{ number_format($activeLeads ?? 0) }} Active</p>
                        <p class="text-xs font-semibold text-gray-700">{{ ($activeLeads ?? 0) > 0 && ($totalLeads ?? 0) > 0 ? round(($activeLeads / $totalLeads) * 100, 1) : 0 }}% of total</p>
                    </div>
        ...
        <div class="group relative overflow-hidden rounded-md bg-gradient-to-br from-orange-50 to-amber-100 p-1.5 shadow-sm ring-1 ring-orange-200/50 transition-all duration-300 hover:shadow-md hover:ring-orange-300/50">
            ...
            <div class="relative">
                ...
                <div class="mt-1">
                    <div class="space-y-0">
                        <p class="text-sm font-bold text-gray-900">AED {{ number_format($totalDealValue ?? 0, 0) }}</p>
                        <p class="text-xs font-semibold text-gray-700">{{ $openDeals ?? 0 }} open deals</p>
                    </div>
        ...
        <div class="group relative overflow-hidden rounded-md bg-gradient-to-br from-purple-50 to-violet-100 p-1.5 shadow-sm ring-1 ring-purple-200/50 transition-all duration-300 hover:shadow-md hover:ring-purple-300/50">
            ...
            <div class="relative">
                ...
                <div class="mt-1">
                    <div class="space-y-0">
                        <p class="text-sm font-bold text-gray-900">{{ number_format($totalWonLeads ?? 0) }} Won</p>
                        @if(($wonGrowthPercentage ?? 0) >= 0)
                            <p class="text-xs font-semibold text-green-600">+{{ $wonGrowthPercentage ?? 0 }}% growth</p>
                        @else
                            <p class="text-xs font-semibold text-red-600">{{ $wonGrowthPercentage ?? 0 }}% decline</p>
                        @endif
                    </div>
                        ...
                        @foreach($pipelineByStage as $stage)
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <p class="text-sm font-medium text-gray-900 capitalize">{{ str_replace('_', ' ', $stage->status) }}</p>
                                        <p class="text-xs text-gray-500">{{ $stage->count }} {{ $stage->count === 1 ? 'lead' : 'leads' }}</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <p class="text-sm font-semibold text-gray-900">AED {{ number_format($stage->total_value ?? 0, 0) }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
