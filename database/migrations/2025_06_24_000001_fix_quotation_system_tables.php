    ...
    public function up(): void
    {
        ...
        if (!Schema::hasTable('quotes')) {
            ...
            Schema::table('quotes', function (Blueprint $table) {
                ...
                if (!in_array('quote_number', $columns)) {
                ...
                if (!in_array('quote_date', $columns)) {
                    $table->date('quote_date');
                }
                if (!in_array('expiry_date', $columns)) {
                    $table->date('expiry_date');
                }
                if (!in_array('salesperson', $columns)) {
                    $table->string('salesperson')->nullable();
                }
                ...
                if (!in_array('terms_conditions', $columns)) {
                    $table->text('terms_conditions')->nullable();
                }
                if (!in_array('status', $columns)) {
                    $table->enum('status', ['draft', 'sent', 'invoiced'])->default('draft');
                }
                if (!in_array('sub_total', $columns)) {
                    $table->decimal('sub_total', 10, 2)->default(0);
                }
